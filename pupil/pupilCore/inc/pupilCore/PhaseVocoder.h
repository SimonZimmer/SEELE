...
namespace sz
{
    ...
    static void
    linearResample(const float* const originalSignal, int originalSize, float* const newSignal, int newSignalSize)
    {
    ...
    public:
        // Default settings
        PhaseVocoder(int windowLength = 2048, int fftSize = 2048) : samplesTilNextProcess(windowLength),
                                                                    ...
                                                                    fft(std::make_unique<juce::dsp::FFT>(
                                                                            nearestPower2(fftSize))),
                                                                    analysisBuffer(windowLength),
                                                                    synthesisBuffer(windowLength * 3),
                                                                    windowBuffer(1, windowLength),
                                                                    spectralBuffer(2, windowLength),
                                                                    resampleBuffer(2, windowLength)
        {
            // TODO make the window more configurable
            juce::dsp::WindowingFunction<FloatType>::fillWindowingTables(windowBuffer.getDataPointer(), windowSize,
            ...
            spectralBufferSize = windowLength * (1 / MinPitchRatio) < spectralBufferSize ? (int) ceil(
                    windowLength * (1 / MinPitchRatio)) : spectralBufferSize;
            spectralBuffer = core::AudioBuffer<float>(2, spectralBufferSize);
            spectralBuffer.fill(0.f);

            // Calculate maximium size resample signal can be
            const auto maxResampleSize = (int) std::ceil(
                    std::max(this->windowSize * MaxPitchRatio, this->windowSize / MinPitchRatio));

            resampleBuffer = core::AudioBuffer<float>(2, maxResampleSize);
        ...
        void process(FloatType* const incomingBuffer, const int incomingBufferSize)
        {
            ...
            for (auto internalOffset = 0, internalBufferSize = 0;
                 internalOffset < incomingBufferSize; internalOffset += internalBufferSize)
            {
                ...
                if (incomingSampleCount >= samplesTilNextProcess)
                {
                    ...
                    analysisBuffer.read(spectralBuffer.getDataPointer(), windowSize);

                    for(auto ch = 0; ch < spectralBuffer.getNumChannels(); ++ch)
                        for(auto sa = 0; sa < spectralBufferSize; ++sa)
                            spectralBuffer[ch][sa] *= windowBuffer[ch][sa];

                    auto spectralBufferPtr = spectralBuffer.getDataPointer();
                    auto windowBufferPtr = windowBuffer.getDataPointer();
                    auto resampleBufferPtr = resampleBuffer.getDataPointer();

                    // Rotate signal 180 degrees, move the first half to the back and back to the front
                    std::rotate(spectralBufferPtr, spectralBufferPtr + (windowSize / 2), spectralBufferPtr + windowSize);

                    // Perform FFT, process and inverse FFT
                    fft->performRealOnlyForwardTransform(spectralBufferPtr);
                    processImpl(spectralBuffer, spectralBufferSize);
                    fft->performRealOnlyInverseTransform(spectralBufferPtr);

                    // Undo signal back to original rotation
                    std::rotate(spectralBufferPtr, spectralBufferPtr + (windowSize / 2), spectralBufferPtr + windowSize);

                    // Apply window to signal
                    juce::FloatVectorOperations::multiply(spectralBufferPtr, windowBufferPtr, windowSize);

                    // Resample output grain to N * (hop size analysis / hop size synthesis)
                    linearResample(spectralBufferPtr, windowSize, resampleBufferPtr, resampleSize);

                    synthesisBuffer.setWriteHopSize(synthesisHopSize);
                    synthesisBuffer.overlapWrite(resampleBuffer.getDataPointer(), resampleSize);
                }

                // Emit silence until we start producing output
                if (!isProcessing)
                {
                    std::fill(incomingBuffer + internalOffset, incomingBuffer + internalOffset + internalBufferSize,
                              0.f);
        ...
        {
            return std::fmod(arg + juce::MathConstants<FloatType>::pi, -juce::MathConstants<FloatType>::twoPi) +
    ...
    private:
        virtual void processImpl(const core::AudioBuffer<float>&, const int) = 0;
    ...
    private:
        ...
        core::AudioBuffer<float> spectralBuffer;
        core::AudioBuffer<float> resampleBuffer;
    ...
    protected:
        ...
        core::AudioBuffer<float> windowBuffer;
