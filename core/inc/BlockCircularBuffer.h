...
namespace sz
{
    ...
    struct BlockCircularBuffer final
    {
        ...
        void read(ElementType* const destBuffer, const long destLength)
        {
            ...
            {
                memcpy(destBuffer + firstReadAmount, internalBuffer,
        ...
        void write(const ElementType* sourceBuffer, const long sourceLength)
        {
            ...
            {
                memcpy(internalBuffer, sourceBuffer + firstWriteAmount,
                       sizeof(ElementType) * (static_cast<unsigned long long>(sourceLength) - firstWriteAmount));
            }
        ...
        void overlapWrite(ElementType* sourceBuffer, const long sourceLength)
        {
            ...
            firstWriteAmount =
                    tempWriteIndex + remainingElements > length ? length - tempWriteIndex : remainingElements;

            memcpy(internalBuffer + tempWriteIndex, sourceBuffer + overlapAmount,
                   sizeof(ElementType) * firstWriteAmount);

            if (firstWriteAmount < remainingElements)
            {
                memcpy(internalBuffer, sourceBuffer + overlapAmount + firstWriteAmount,
